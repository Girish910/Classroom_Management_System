...
<form action="" method="post" enctype="multipart/form-data" name="frm_usr">
...
<tr>
   <td> <label for="syllabus"><b>Syllabus</b></label></td>
    <td><input type="file" id="syllabus" name="sylabus" required>  <br><i><b> <font color="red" size="2px">*you can only upload Photo</font></b></i></td>
...
if(isset($_POST['submit']))
    {
      ...
      $st="select * from metirials where class='$class'";
      $stm=mysqli_query($db,$st);
      while($rows3=mysqli_fetch_assoc($stm))
       {
         
          if(strtoupper($rows3['sub_name'])==strtoupper($subject))
            {

              echo "<script>
              alert('Syllabus of this Subject is ALREADY UPLOADED by someone');
            ...
            if(strtoupper($rows3['sub_code'])==strtoupper($code))
            {

              echo "<script>
              alert('Syllabus of this Subject code is ALREADY UPLOADED by someone');
